...
const SeriesDetails = () => {
    ...
    const getDetails = async (videoId) => {
        const response = await axios.get(`${baseUrl}/tv/${videoId}?api_key=${apiKey}&language=${language}`);
        setDeatils(response.data)
    }

    // Get Movie Credits
    const getMovieCredits = async (videoId) => {
        const response = await axios.get(`${baseUrl}/tv/${videoId}/credits?api_key=${apiKey}&language=${language}`);
        setCredits(response.data)
    }

    // Get Videos and Image Urls
    const getVideosAndImageUrl = async (videoId) => {
        const response = await axios.get(`${baseUrl}/tv/${videoId}/videos?api_key=${apiKey}&language=${language}`)
    ...
    const getPhotos = async (videoId) => {
        const response = await axios.get(`${baseUrl}/tv/${videoId}/images?api_key=${apiKey}&language=${language}`)
        setPhotos(response.data.backdrops)
    }

    // Get Recommended Movies
    const getRecommendedMovies = async (videoId) => {
        const response = await axios.get(`${baseUrl}/tv/${videoId}/recommendations?api_key=${apiKey}&language=${language}&page=1`)
    ...
    return (
        <div className='deatils-conatiner'>
            ...
            <div className="movie-details w-[95%] h-full m-auto pt-4 overflow-x-scroll md:overflow-hidden scrollbar">
                    ...
                    <div className="movie-details-overview-content-wrapper flex flex-col">
                        <div className="upper flex flex-col md:flex-row">
                            <div className="left movie-details-overview-content-wrapper-image text-2xl text-white md:pr-[5rem] ">
                                <div className="img-contrainer md:w-[20rem]">
                                <img className='w-full h-full rounded-lg md:rounded-none    ' src={"https://image.tmdb.org/t/p/original" + detail?.poster_path} alt="" />
                                </div>
                            </div>
                            <div className='right'>
                        ...
                        <div className="lower cast">
                            ...
                            <div className="cast-list flex gap-4">
                                ...
                                >
                                    {
                                        credits?.cast?.length === 0 ? <div className="text-white text-2xl font-bold">No cast found</div> :
            ...
            }
            {
                isVideos && <div className="videos-container flex gap-2 md:gap-10 flex-wrap w-[95%] m-auto mt-12 justify-center border-b-2 border-black border-opacity-20 pb-[2rem]">
                    {
                        videoInfo?.length === 0 ? <div className="text-white text-2xl font-bold">No Videos Found </div> :
                        videoInfo?.map((video) => {
            ...
            }
            {
                isPictures && <div className="pictures-container flex gap-4 flex-wrap w-[95%] m-auto mt-12 justify-center border-b-2 border-black border-opacity-20 pb-[2rem]">
                    {
                        photos?.length === 0 ? <div className="text-white text-2xl font-bold">No Photos Found</div> :
                        photos?.map((photo) => {
            ...
            }
            {
                isSeasons && <div className="seasons-container flex gap flex-wrap w-[95%] m-auto mt-12 justify-center">
                    {
                        detail?.seasons?.length === 0 ? <div className="text-white text-2xl font-bold">No Seasons Found</div> :
                        detail?.seasons?.map((season) => {
                            return <div className="sm:max-w-lg sm:mx-auto md:mx-[1rem]" key={season?.season_number}>
                                    <div className=" shadow-lg border sm:rounded-3xl p-2 mb-12 flex space-x-10 rounded-lg md:rounded-none">
                                        <div className="flex items-center md:inline-block overflow-visible w-1/2">
                                            <Link to={`/${params?.id}/episodes/${season?.season_number}`}><img className="rounded-3xl shadow-lg" src={"https://image.tmdb.org/t/p/w780" + detail?.poster_path} alt="" /></Link>
                                        </div>
                                        <div className="flex flex-col w-1/2 space-y-4 pt-6">
                                            <div className="flex justify-between items-start">
                                            <Link to={`/${params?.id}/episodes/${season?.season_number}`}><h2 className="text-3xl text-white font-bold">{season?.name}</h2>
                                            </Link>
                                            </div>
                                            <div>
                                                <div className="text-sm text-gray-200">Series</div>
                                                <div className="text-lg text-gray-200">{season?.air_date}</div>
                                            </div>
                                            <p className=" text-gray-200 max-h-40 overflow-y-hidden">{season?.overview}</p>
            ...
            <div className="recommanded-video w-[95%] m-auto">
                ...
                <div className="cast-list flex gap-4">
                    ...
                    >
                        {
                            recommandedMovies?.length === 0 ? <div className="text-white text-2xl font-bold">No Recommanded Series</div> :
